...
let userPoint = L.layerGroup().addTo(myMap);
...
function success(pos) {
   ...
   L.marker([latitude, longitude], {icon: home}).addTo(userPoint).bindPopup('You are here!');
...
$.ajax({
   ...
   success: function(result) {
      ...
      alert('Click OK to continue. Then click on a Marker!');
   },
   error: function (jqXHR, textStatus, errorThrown) {
      console.log(errorThrown);
      console.log(textStatus);
   ...
   data: {
     ...
     success: function (flyResult) {
      ...
      $('#countryDropdown').change(function() {
         ...
         if (wikiMarker, cityMarker) {
            myMap.removeLayer(wikiMarker, cityMarker);
         }
      });
...
$.ajax({
   ...
   success: function(infoResult) {
      ...
      // Wikipedia button function.
            let countrySplit = infoResult['data'][0]['countryName'].split(" ").join("%20");
            $.ajax({
      ...
      $.ajax({
         ...
         success: function(weatherResult) {
            ...
            $('#capName').html('The Weather Symbol for ' + infoResult['data'][0]['capital'] + ' today is:') + $('#capWeatherIcon').attr('src', 'https://openweathermap.org/img/wn/' + weatherResult['data']['list'][0]['weather'][0]['icon'] + '@2x.png');
      },
            error: function(jqXHR, textStatus, errorThrown) {
            console.log(jqXHR);
            console.log(textStatus);
      ...
      $.ajax({
         ...
         success: function(newsResult) {
            ...
            $('#newsHead').html(newsResult['data']['articles'][0]['title']);
            $('#news').html('<a href="https://' + newsResult['data']['articles'][0]['url'] + '">'  + newsResult['data']['articles'][0]['url'] + '</a>');

            $('#newsHead1').html(newsResult['data']['articles'][1]['title']);
            $('#news1').html('<a href="https://' + newsResult['data']['articles'][1]['url'] + '">'  + newsResult['data']['articles'][1]['url'] + '</a>');

            $('#newsHead2').html(newsResult['data']['articles'][2]['title']);
            $('#news2').html('<a href="https://' + newsResult['data']['articles'][2]['url'] + '">'  + newsResult['data']['articles'][2]['url'] + '</a>');

            $('#newsHead3').html(newsResult['data']['articles'][3]['title']);
            $('#news3').html('<a href="https://' + newsResult['data']['articles'][3]['url'] + '">'  + newsResult['data']['articles'][3]['url'] + '</a>');

            $('#newsHead4').html(newsResult['data']['articles'][4]['title']);
            $('#news4').html('<a href="https://' + newsResult['data']['articles'][4]['url'] + '">'  + newsResult['data']['articles'][4]['url'] + '</a>');
...
$.ajax({
 ...
 $.ajax({
   ...
   for (let c = 0; c < cityResult.data.geonames.length; c++) {
      L.marker([cityResult['data']['geonames'][c]['lat'],
      cityResult['data']['geonames'][c]['lng']], {icon: cityMarker})
      .on('click', markerClick).addTo(markerCluster);

      function markerClick() {
      
         $('#myModal5').modal('show');
         $('.modal-title1').html(cityResult['data']['geonames'][c]['name']);
         $('#cityPop').html('The City Center Population is: ' + cityResult['data']['geonames'][c]['population']);
         ...
         $.ajax({
            ...
            success: function(result) {
               ...
               $('#cityTemp').html('The current temperature for ' + cityResult['data']['geonames'][c]['name'] + ' is ').append(result['data']['current']['temp'] + ' Celsius');
               $('#cityDes').html('The weather description for  ' + cityResult['data']['geonames'][c]['name'] + ' is ').append(result['data']['current']['weather'][0]['description']);
               $('#cityLoc').html('The City`s Weather Symbol is: ')  + $('#cityWeatherIcon').attr('src', 'https://openweathermap.org/img/wn/' + result['data']['current']['weather'][0]['icon'] + '@2x.png');
            }, 
            error: function (jqXHR, textStatus, errorThrown) {
               console.log(errorThrown);
               console.log(textStatus);
