            ...
            success: function(result) {
                ...
                for (let i = 0; i < result.data.length; i++) {
                    $('#locationDisplay').append(
                        '<div class="col mb-4"><div class="card"><i class="fas fa-location-arrow fa-4x pt-2 m-auto"></i><div class="card-body"><h5 class="card-title text-center fw-bold fs-5">Location</h5><table class="table"><tbody><tr><td class="text-center">' + result['data'][i]['name'] + '</td></tr></tbody></table><button id="delLoc" type="button" value="' + result['data'][i]['id'] + '" class="btn btn-secondary mainButtons" data-bs-toggle="modal" data-bs-target="#deleteLoc" title="Delete Location"><i class="far fa-trash-alt fa-lg"></i></button><button type="button" value="'+ result['data'][0]['id'] +'" class="btn btn-secondary mainButtons" data-bs-toggle="modal" data-bs-target="#editLocation" title="Edit Location"><i class="far fa-edit fa-lg"></i></button></div></div></div>'
                    );
                }// End of loop
                    ...
                    $('#locationBtn').click();
                        ...
                        $.ajax({
                            ...
                            success: function(result) {
                                console.log(result);
                                $.each(result.data, function(i) {
                                    $('#delLocation, #locToEdit').append($('<option>', {
                                    text: result.data[i].name,
                                    value: result.data[i].id
                                    }
                                   ));
